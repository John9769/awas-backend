    ...
    try {
        const pending = await prisma.institutionalUser.findMany({
            ...
            select: {
                ...
                requesterType: true, licenseId: true, createdAt: true
            }
        });
...
exports.getAllInstitutions = async (req, res) => {
    try {
        const all = await prisma.institutionalUser.findMany({
            select: {
                id: true, email: true, companyName: true,
                requesterType: true, licenseId: true,
...
exports.approveInstitution = async (req, res) => {
    try {
        ...
        res.status(200).json({
            ...
            id: approved.id, isApproved: approved.isApproved
        });
    } catch (error) {
...
exports.revokeInstitution = async (req, res) => {
    try {
        ...
        res.status(200).json({
            ...
            id: revoked.id, isApproved: revoked.isApproved
        });
    } catch (error) {
...
exports.getDashboard = async (req, res) => {
    try {
        const [
            ...
            prisma.accidentLog.count({ where: { injuryStatus: { not: 'NONE' } } }),
            prisma.accidentLog.count({ where: { otherVehiclePlate: { not: null } } }),
